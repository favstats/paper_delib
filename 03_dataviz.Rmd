...
ggsave(filename = "text/images/delib_polity.png", width = 12, height = 6)
```
...
ggsave(filename = "text/images/doubleplot.png", width = 12, height = 6)
```
...
ggsave(filename = "text/images/regimesupport_fod.png", width = 12, height = 6)
...
#  scale_fill_gradient(low = "red", high = "blue") + 
  coord_equal() +
  viridis::scale_fill_viridis("Average Value", 
...
ggsave(filename = "images/map3.png", height = 6, width = 12) 
...
```{r}
load("text/data/estimate_data.Rdata")
...
controls_plot %>% 
 ggplot() + 
  ...
  geom_text(aes(x = term,
                y =  -.23,
                label = model),
            show.legend = F, nudge_x = 0.4, size = 2.5, color = "gray20") +
  ...
  scale_y_continuous(breaks = seq(x_min, 0.475, 0.10), 
                     limits = c(x_min, 0.475), labels =  round(seq(x_min, 0.475, 0.10), 2))  +
  # cowplot::background_grid(major = 'xy', minor = "xy") + # add thin horizontal lines
  # geom_vline(xintercept = seq(1.5, length(unique(controls_plot$term)) - 0.475, 1), 
...
rath::ggpreview(gg_controls, width = 8, height = 9)
...
with_pols_dat %>% 
 ggplot() + 
  ...
  geom_text(data = with_pols_dat_delibs, aes(x = term,
                y =  -.23,
                label = model),
            show.legend = F, nudge_x = 0.32, size = 2.5, color = "gray20") +
  ...
  # geom_vline(xintercept = seq(4.1, 22 - 0.475, 4),
  #            lwd = 1, colour = "grey", alpha = 0.7) + # adds neat lines between factors
  scale_x_discrete(limits = rev(c("Deliberation Component Index",
                                      "Democracy (DCI)",
                                      "Autocracy (DCI)",
  ...
  scale_y_continuous(breaks = seq(x_min, 0.475, 0.1), 
                     limits = c(x_min, 0.475), labels =  round(seq(x_min, 0.475, 0.1), 2)) +
  xlab("") + 
  ylab("Estimate") +
...
controls_plot_dem %>% 
 ggplot() + 
  ...
  geom_text(aes(x = term,
                y =  -.23,
                label = model),
            show.legend = F, nudge_x = 0.25, size = 2.5, color = "gray20") +
  # geom_text(data = with_controls_icc, 
  #           aes(x = term, 
  #               y =  0.25,
  #               label = paste0("ICC: ", sprintf('%.2f', icc, 2))), 
 ...
 ggplot() + 
  ...
  scale_color_manual("Dependent Variable", 
                               labels = rev(c("Regime Support (No Bias)", 
                                          ...
                                          "Regime Support (High Bias)")),
                       values = rev(gdocs_pal()(3))) +
  scale_y_continuous(breaks = seq(x_min, 0.4, 0.10), 
                     limits = c(x_min, 0.4), labels =  round(seq(x_min, 0.4, 0.10), 2))  +
  # cowplot::background_grid(major = 'xy', minor = "xy") + # add thin horizontal lines
  # geom_vline(xintercept = seq(1.5, length(unique(controls_plot$term)) - 0.475, 1), 
  #            lwd = 1, colour = "grey", alpha = 0.7) + # adds neat lines between factors
  scale_x_discrete(limits = rev(c("Deliberation Component Index",
...
with_pols_dat_nondem %>% 
 ggplot() + 
  ...
  geom_text(data = with_pols_dat_delibs_nondem, aes(x = term,
                y =  -.32,
                label = model),
            show.legend = F, nudge_x = 0.69, size = 2.5, color = "gray20") +
  ...
  scale_x_discrete(limits = rev(c("Deliberation Component Index",
                   ...
                   labels = c("Deliberation Component Index"=expression(displaystyle(bold(Deliberation~~Component~~Index))), 
                            ...
                            "Democracy (ES)"=expression(displaystyle(italic(Polity/FH))), 
                            parse=TRUE), expand = c(0, 1.1)) +
  scale_y_continuous(breaks = seq(x_min, 0.4, 0.1), 
                     limits = c(x_min, 0.4), labels =  round(seq(x_min, 0.4, 0.1), 2)) +
  xlab("") + 
  ylab("Estimate") +
  ggtitle("Individual and Macro Level Controls + Polity/FH") + 
...
just_pols_dat %>% 
 ggplot() + 
  ...
  theme(plot.title = element_text(hjust = 0.475)) -> gg_just_pols
...
just_pols_dat_dem %>% 
 ggplot() + 
  ...
  theme(plot.title = element_text(size=14, face="bold.italic"))  + 
  guides(colour = guide_legend(reverse=T)) +
  theme(plot.title = element_text(hjust = 0.475)) -> gg_just_pols_dem
...
just_pols_dat_nondem %>% 
 ggplot() + 
  ...
  theme(plot.title = element_text(size=14, face="bold.italic"))  + 
  guides(colour = guide_legend(reverse=T)) +
  theme(plot.title = element_text(hjust = 0.475)) -> gg_just_pols_nondem
...
controls_plot_dem %>% 
  mutate(sample = "Democracy Sample") %>% 
  bind_rows(controls_plot %>% 
            mutate(sample = "Complete Sample")) %>% 
  bind_rows(controls_plot_nondem %>% 
            mutate(sample = "Non-Democracy Sample")) %>% 
  filter(term_cite == "delib10") %>% 
 ggplot() + 
  aes(x = term, y = estimate, color = bias) + 
  geom_hline(yintercept = 0, 
             color = "gray25", 
  ...
  geom_text(aes(x = term, 
                y = estimate + 0.16,
                label = paste(sprintf('%.2f', estimate, 2), stars)), 
                position = position_dodge(width = .9), size = 5.2,
            show.legend = F) +
  geom_text(aes(x = term,
                y =  -.23,
                label = model),
            show.legend = F, nudge_x = 0.4, size = 3.5, color = "gray20") +
  # geom_text(data = with_controls_icc, 
  #           aes(x = term, 
  #               y =  0.25,
  #               label = paste0("ICC: ", sprintf('%.2f', icc, 2))), 
  ...
  theme(plot.title = element_text(size=14, face="bold.italic"),
        ...
        strip.text = element_text(size = 12)) + 
  guides(colour = guide_legend(reverse=T)) +
  facet_wrap(~sample, ncol = 1) -> control_slides


ggsave(control_slides, filename = "images/control_slides.png", width = 8, height = 9)
```
...
```{r}
estimates_data %>% 
  filter(set == "with_pols_dat_dem") %>% 
  mutate(sample = "Democracy Sample") -> with_pols_dat_dem

estimates_data %>% 
  filter(set == "with_pols_dat_nondem") %>% 
  mutate(sample = "Non-Democracy Sample")  -> with_pols_dat_nondem


estimates_data %>% 
  filter(set == "with_pols_dat") %>% 
  mutate(sample = "Complete Sample") %>% 
  bind_rows(with_pols_dat_dem) %>% 
  bind_rows(with_pols_dat_nondem) %>% 
  filter(term_cite %in% "delib10" | 
           term %in% c("Democracy (DCI)", "Autocracy (DCI)")) -> with_pols_dat
...
with_pols_dat %>% 
  filter(term %in% c("Democracy (DCI)",
                     "Autocracy (DCI)")) -> with_pols_dat_regime


with_pols_dat %>% 
  filter(term %nin% c("Democracy (DCI)",
                     "Autocracy (DCI)")) -> with_pols_dat_delibs
...
with_pols_dat %>% 
 ggplot() + 
  ...
  geom_text(data = with_pols_dat_delibs, aes(x = term, 
                y = estimate + 0.19,
                label = paste(sprintf('%.2f', estimate, 2), stars)), 
                position = position_dodge(width = 1.1), size = 5.2,
            show.legend = F) +
  geom_text(data = with_pols_dat_regime, aes(x = term, 
                y = estimate + 0.19,
                label = paste(sprintf('%.2f', estimate, 2), stars)), 
  ...
  theme(plot.title = element_text(size = 14, face="bold.italic"),
        axis.title.y = element_text(size = 21),
        strip.text = element_text(size = 12)) -> slide_pols


ggsave(slide_pols, filename = "images/slide_pols.png", width = 8, height = 9)
```
...
```{r}
legend_b <- cowplot::get_legend(control_slides + theme(legend.direction = "horizontal",
                                                   legend.justification="center",
                                                   legend.box.just = "bottom"))

library(gridExtra)

grid.arrange(control_slides  + theme(legend.position="none"),
                   slide_pols + theme(legend.position="none"), legend_b, ncol=2, nrow = 2, 
             layout_matrix = rbind(c(1,2), c(3,3)),
             widths = c(2.7, 2.7), heights = c(2.5, 0.2)) -> coefplot

ggsave(coefplot, filename = "slides/images/slides_coefplot.png", width = 18, height = 10)
...
# Just Control Models vs. Just Deliberation Models


full_anovies_wo_delibs %>% 
  mutate(comparison = case_when(
    str_detect(comparison, "Complete") ~ "Complete Sample",
    str_detect(comparison, "Non-Dem") ~ "Non-Democracy Sample",
    str_detect(comparison, "Dem. Sample") ~ "Democracy Sample"
  )) %>% 
 ggplot(aes(bias, type, fill = stars)) +
   geom_tile(color = "white") +
   scale_fill_manual("Likelihood Ratio Test\n Significance Level", 
...
ggsave(filename = "slides/images/model_comparison_wo_delibs2.png", width = 15, height = 8)
...
bind_rows(
full_anovies %>% 
  filter(str_detect(comparison, "Polity")) %>% 
  mutate(sample = "Complete Sample"),
...
ggsave(filename = "images/model_comparison_polities.png", width = 15, height = 8)
