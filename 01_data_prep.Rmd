...
#p_install_gh("favstats/tidytemplate")
p_load(tidyverse, haven, sjmisc,
                     forcats, weights, car,
                     countrycode, lavaan,
                     semTools, lavaan.survey,
                     survey, reshape2, tidytemplate,
                     magrittr, skimr, xray, wbstats)
...
```{r}
wbsearch(pattern = "life expectancy")


growth_data <- wb(indicator = "NY.GDP.MKTP.KD.ZG", startdate = 2000, enddate = 2010)
gdp_data <- wb(indicator = "NY.GDP.PCAP.PP.CD", startdate = 2000, enddate = 2010)
life_data <- wb(indicator = "SP.DYN.LE00.IN", startdate = 2000, enddate = 2010)


	

gdp_data <- gdp_data %>% 
  select(iso3c, gdp10 = value) %>% 
  group_by(iso3c) %>% 
  summarise_all(mean) %>% 
  arrange(gdp10) %>% 
  mutate(cntry = countrycode(iso3c, "iso3c","country.name")) %>% 
  drop_na(cntry)

growth_data <- growth_data %>% 
  select(iso3c, gdp_growth10 = value) %>% 
  group_by(iso3c) %>% 
  summarise_all(mean) %>% 
  mutate(cntry = countrycode(iso3c, "iso3c","country.name")) %>% 
  drop_na(cntry) %>% 
...
level2 <- merge(x = qog10, y = vdem, by = "cntry") %>% 
  select(-gdp10, -lifeexp10) %>% 
  merge(gini, by = "cntry") %>% 
  merge(growth_data, by ="cntry") %>% 
  merge(gdp_data, by ="cntry") %>% 
  merge(life_data, by ="cntry")
...
combined %>% 
  select(cntry, gdp_growth10) %>% 
  group_by(cntry, gdp_growth10) %>% 
  summarise_all(mean)


combined %>% count(cntry)
```
...
combined <- combined %>% 
  ...
  select(cntry, year, gov_trust, gov_trust_low, gov_trust_high, 
        age:work, survey:ESS, delib10:engage10, polity10,
        polity_demdummy, polity_anodummy, polity_autodummy, regime,
       discuss, discuss_round, gdp10, lifeexp10,  pop10, urbanratio10, gdp_growth10, weight) %>% 
#   sample_frac(0.01) %>%
#   visdat::vis_miss(cluster = T, sort_miss = T, show_perc = T)
# rath::ggpreview(combined2, width = 12, height = 6)
                ...
                #        urbanratio10, gdp_growth10, weight) %>% 
  na.omit() %>%
...
macro  <- combined %>% 
                ...
                select(cntry, mean_gov, mean_gov_low, mean_gov_high, 
                       ...
                       poly10, riw10, unified_dem10, urbanratio10, gdp_growth10) %>% 
                drop_na(mean_gov) %>% 
                mutate(count_na = apply(., 1, count_na)) %>% 
  mutate_at(vars(delib10:polity10, polity_demdummy:polity_autodummy, discuss:urbanratio10, gdp_growth10), range01)
