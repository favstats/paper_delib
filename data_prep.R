...
lubripack::lubripack("tidyverse", "haven","sjmisc",
                     "forcats","weights", "car",
                     "countrycode", "lavaan",
...
#afro5 %>% mutate(work = Recode(Q96, #work (0/1) codieren
#                         "1 = '0';
#                          2 = '1';
#                          3 = '1'")) %>% select(work) %>% table()
...
#P16TGB.B Confidence in groups-institutions-people: Police
#P16ST.F Confidence in groups-institutions-people: National Congress/Parliament
#P16ST.G Confidence in groups-institutions-people: The National Government
#P16ST.H Confidence in groups-institutions-people: Judiciary
...
#q8. The courts
#q9. The national government [in capital city]
#q11. Parliament
#q14. The polic
...
taiwan_raw %>%
  rename(educ = se5,
         ...
         trust_courts = q8,
         sex = se2,
         income = se14a, ## income ist leer und das ist dreiteilig
         work = se9) %>%
  mutate_at(vars(income, trust_gov, trust_parliament, 
                 trust_police, trust_courts, work, sex), 
            delete_na_asian) %>%   #NAs deleten 
...
table(asian$educ)
table(asian$income)
table(asian$demtoday)
table(asian$trust_gov)
table(asian$trust_parliament)
table(asian$trust_police)
table(asian$trust_courts)
table(asian$sex) #MISSING VALUES!
table(asian$work)
table(asian$age)
...
#euro <- euro %>% dplyr::select(cntry,gov_trust,survey,demtoday,
#                                       trust_gov, trust_parliament, 
#                                       trust_police,trust_courts,
#                                       year)
...
#merged<-subset(merged,merged$cntry!="Egypt") #exclude Egypt 2013
#merged<-subset(merged,merged$cntry!="Libya" | merged$year!=2014) #exclude Libya 2014
#merged<-subset(merged,merged$cntry!="Mali") #exclude Mali 2012
...
# merged2 <- subset(merged,!is.na(merged$gov_trust))
# merged3 <- subset(merged,is.na(merged$gov_trust))
...
cor(na.omit(data.frame(na.omit(merged$gov_trust),
                       merged4$gov_trust2)))

ggplot(merged,aes(gov_trust, cntry)) + geom_beeswarm(cex=1.2,groupOnX=FALSE)


#### LEVEL 2 ####
load(vdems_start_url)

#table(vdems_start$year)
#
#table(vdems_start$v2dlconslt,vdems_start$year)
#
...
cntry <- unique(vdems_sub$country_name)
...
#data_wide13 <- dcast(vdems_sub, country_name ~ year, 
#                     value.var=c("e_Total_Oil_Income_PC"))
#
#data_wide14 <- dcast(vdems_sub, country_name ~ year, 
#                     value.var=c("e_Coal_Income_PC"))
#
#data_wide15 <- dcast(vdems_sub, country_name ~ year, 
#                     value.var=c("e_natural_gas_income_PC"))

#data_wide9$country_name<-countrycode(data_wide9$country_name,"country.name.en","country.name.en")
#data_wide9$country_name[40] <- "Democratic Republic of Vietnam"
...
aggrdelib <- data.frame(cntry,consult10,delib10,reason10,educ10,
                        common10,countr10,engage10,delibdem10,gini10,
                        polity10,riw10,corecivil10,gdp10,pop10,
                        corruption10,regions,regions2,regions3,
                        postcom,latin,mena,subsahara, polkill10,
                        west,e.asia,s.e.asia,s.asia,pacific)

aggrdelib$polity10 <- (aggrdelib$polity10*2)-10

#min(aggrdelib$polity10,na.rm=T)

unique(aggrdelib$cntry)


aggrdelib$cntry<-countrycode(aggrdelib$cntry,"country.name.en","country.name.en")
aggrdelib$cntry[40] <- "Viet Nam"

aggrdelib$polis <- round(aggrdelib$polity10)


aggrdelib$polity_demdummy <- aggrdelib$polis
aggrdelib$polity_demdummy [aggrdelib$polis > 5] <- 1
aggrdelib$polity_demdummy [aggrdelib$polis <=  5] <- 0
table(aggrdelib$polity_demdummy)

aggrdelib$polity_anodummy <- aggrdelib$polis
aggrdelib$polity_anodummy[aggrdelib$polis >= -5 | aggrdelib$polis <= 5] <- 1
aggrdelib$polity_anodummy[aggrdelib$polis < -5 | aggrdelib$polis > 5] <- 0
table(aggrdelib$polity_anodummy)

aggrdelib$polity_autodummy <- aggrdelib$polis
aggrdelib$polity_autodummy[aggrdelib$polis < -5] <- 1
aggrdelib$polity_autodummy[aggrdelib$polis > -5] <- 0
table(aggrdelib$polity_autodummy)

aggrdelib$regime <- aggrdelib$polis
aggrdelib$regime[aggrdelib$polity_autodummy==1] <- "auto"
aggrdelib$regime[aggrdelib$polity_anodummy==1] <- "ano"
aggrdelib$regime[aggrdelib$polity_demdummy==1] <- "demo"

print(levels(aggrdelib$regime)) 
aggrdelib$regime<-factor(aggrdelib$regime, levels = c("ano","auto","demo"))
aggrdelib$regime<-factor(aggrdelib$regime,levels(aggrdelib$regime)[c(2,1,3)])

table(aggrdelib$regime)
...
aggrdelib <- merge(x=qogthingy, y=aggrdelib, by="cntry")
...
combined$cntryears<-paste(combined$cntry,combined$year.x)

vdems_sub2 <- subset(vdems_start,vdems_start$year==2010 |
                     vdems_start$year==2011 |  
                     vdems_start$year==2012 | 
                     vdems_start$year==2013 | 
                     vdems_start$year==2014 | 
                     vdems_start$year==2015)

#table(vdems_sub2$v2x_clphy,vdems_sub2$country_name)

vdems_sub2$perc <- vdems_sub2$v2x_clphy
#vdems_sub2$franz <- vdems_sub2$e_fh_ipolity2
#vdems_sub2$franz <- (vdems_sub2$franz*2)-10

#min(vdems_sub2$franz, na.rm=t)

vdems_sub2$cntry<-countrycode(vdems_sub2$country_name,"country.name.en","country.name.en")
vdems_sub2$cntry[996] <- "Viet Nam"
vdems_sub2$cntry[997] <- "Viet Nam"
vdems_sub2$cntry[998] <- "Viet Nam"
vdems_sub2$cntry[999] <- "Viet Nam"
vdems_sub2$cntry[1000] <- "Viet Nam"
vdems_sub2$cntry[1001] <- "Viet Nam"
...
vdems_sub2$perm <- vdems_sub2$v2xcl_disc

physi_2 <- vdems_sub2 %>% dplyr::select(cntryears, perm)

combined <- merge(combined, physi_2, by = "cntryears")

physi_22 <- ddply(combined,~cntry,
                summarise,discuss=mean(perm,na.rm=T))

combined <- merge(combined, physi_22, by = "cntry")
...
save(aggrdelib,file="aggrdelib.Rdata")

table_stuff  <- combined %>% dplyr::select(cntry, gov_trust,
                                           demtoday,delib10,polity10)
table_stuff$gov_trust <- range01(table_stuff$gov_trust)*100
table_stuff$demtoday <- range01(table_stuff$demtoday)*100

table_stuff <- ddply(table_stuff,~cntry,
                     summarise,mean_gov=mean(gov_trust,na.rm=T),
                     mean_dem=mean(demtoday,na.rm=T))

table_stuff <- merge(table_stuff, aggrdelib, by="cntry")
table_stuff <- merge(table_stuff, physi2, by="cntry")
table_stuff <- merge(table_stuff, physi22000, by="cntry")
table_stuff <- merge(table_stuff, physi_22, by="cntry")
table_stuff <- merge(table_stuff, physigni2, by="cntry")
#table_stuff <- merge(table_stuff, physiff2, by="cntry")
...
cmpr <- table_stuff %>% filter(discuss2 < 2) %>% 
  dplyr::select(cntry,mean_gov6,mean_gov7,mean_gov8,mean_gov9)
...
#cmpr <- table_stuff %>% filter(discuss2 <= 2) %>% dplyr::select(cntry,mean_gov6,mean_gov7,mean_gov8,mean_gov9)


#cus <- combined %>% filter(discuss2 <= 2) %>% ddply(~cntry,
#                     summarise,gov_trust8=mean(gov_trust8,na.rm=T),
#                      gov_trust9=mean(gov_trust9,na.rm=T))

#cmpr<-merge(cmpr,cus,by="cntry")

merged$cntryears <- paste(merged$cntry,merged$year)

breakdown <- read_csv("breakdown.csv")

breakdown$cntry<-countrycode(breakdown$COUNTRY,"country.name.en","country.name.en")

breakdown$cntryears <- paste(breakdown$cntry,breakdown$YEAR)

fruitful <- merge(merged,breakdown, by="cntryears")

table(fruitful$MAGFAIL,fruitful$cntryears)
egypt<-filter(merged, cntry=="Egypt")
table(merged$cntry,merged$wvs)
